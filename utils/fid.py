...
class FID():
    ...
    def cal_fid_score(self, pred, gt):
        fake_act = get_activations(pred, self.inception_model_fid, self.new_batch_size)
        real_act = get_activations(gt, self.inception_model_fid, self.new_batch_size)
        real_mu, real_sigma = calculate_activation_statistics(real_act)
        fake_mu, fake_sigma = calculate_activation_statistics(fake_act)
        fid_score = calculate_frechet_distance(real_mu, real_sigma, fake_mu, fake_sigma)
        return fid_score

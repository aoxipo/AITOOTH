...
@register_model
class pvt_v2_b2_one_layer(PyramidVisionTransformerImpr):
    def __init__(self, **kwargs):
        super(pvt_v2_b2_one_layer, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 0, 0, 0], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)
...
class PyramidVisionTransformer(nn.Module):
    ...
    # def init_weights(self, pretrained="./pretrained_pth/pvt_tiny.pth"):
    #     if isinstance(pretrained, str):
...
#             img_size=kwargs.get("img_size",224), patch_size=4, embed_dims=kwargs.get("embed_dims",[64, 128, 320, 512]), num_heads=kwargs.get("num_heads",[1, 2, 5, 8]), mlp_ratios=[8, 8, 4, 4],
...
def pvt_tiny(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt = torch.load('./pretrained_pth/pvt_large_iter_40000.pth')

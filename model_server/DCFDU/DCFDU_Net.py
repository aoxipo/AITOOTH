...
from .ddpm.dd import GaussianDiffusionSampler
import cv2
import numpy as np
...
class DCFDU(nn.Module):
    def __init__(self, img_ch  =  1, output_ch  =  1, 
        ...
        self.last_decode_levelset = nn.Sequential(
            nn.Conv2d(middle_channel[next_dim], 128, kernel_size = 3, stride = 1, padding = 1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
    ...
    def build_results(self, x, y,z, middle = None, super_vision  = None):
    ...
    def forward(self, x, t = None):
        ...
        edge = self.edge_hot_map(out_level_set)
        ...
        if self.need_supervision:
            ...
            return self.build_results(outp, out_level_set, edge, supervision) if self.need_return_dict else( outp, edge, supervision) 
        return self.build_results(outp, out_level_set, edge) if self.need_return_dict else( outp, edge ) 

ddpmSamplerhelper = GaussianDiffusionSampler(None, 0.01,0.04,20).cuda()
from .ddpm.dd import Boundary
class DCFDU_with_ddpm_boundary(nn.Module):
    def __init__(self, img_ch  =  1, output_ch  =  1, 
                middle_channel = [64, 128, 256, 512, 1024], 
                encode_len = 5, 
                need_return_dict = True,
        ...
        super(DCFDU_with_ddpm_boundary, self).__init__()
        pvt_channel = [64, 128, 320, 512]
        self.index_len = encode_len - 1
        self.need_return_dict = need_return_dict
        self.downsample = nn.AvgPool2d(2)
        ...
        self.bround_ddpm = Boundary(1024)
        self.fill_helper()
        self.backbone = pvt_v2_b2()  # [64, 128, 320, 512]

    def fill_helper(self):
        ddpmSamplerhelper.model = self.bround_ddpm
    ...
    def edge_hot_map(self, x1):
        x = x1.clone().detach()
        level_set_index = ddpmSamplerhelper(x).squeeze()
        B,_,_,_ = x.shape
        ...
        # cv2.imwrite( './test.png',np.array(edge[0].detach().cpu().squeeze().numpy() * 255, np.uint8))
        return edge
